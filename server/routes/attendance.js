...
router.get('/list', verifyToken, (req, res) => {
  ...
  const sql = `SELECT attendance.id, attendance.user_id, attendance.date, attendance.check_in, attendance.check_out, attendance.status, users.name, users.email
...
router.get('/my', verifyToken, (req, res) => {
  const sql = `SELECT id, date, check_in, check_out, status FROM attendance WHERE user_id = ? ORDER BY date DESC LIMIT 100`;
